...
$tags = {
"bigbrother" => %w|腾讯 百度 新浪 华为 阿里 牌照 金融 微信 来往 微软 谷歌 京东 美团 特斯拉 中国移动 中国联通 电信 大众点评 酒仙网 银联 IBM HP 余额宝 央行 土豆 1号店 苹果 英特尔 4G 中兴 九城 YY 小米 红米 MIUI Google 海尔 微博 携程 赶集 58同城 金钱豹 支付宝 银行 Facebook 星巴克 联想 HTC|,
...
"relax" => %w|姚晨 谢娜 李代沫|,
"startup" => %w|创始人 创业 融资 大姨吗 无印良品 电商 商业模式|,
"code" => %w|编程 编码 程序员 极客 云计算 大数据 可穿戴 智能电视 智能家居 智能硬件 软件 Erlang Linux 软件 代码 BUG bug 图灵 Java java python ruby Ruby Python JAVA 智能手表|,
"cool" => %w|创意 创造力 非常酷 太有才 泡妞 Oculus 天才|,
...
public
  ...
  def all
  	...
  	@hot_artical = Content.find_by_sql("select * from contents join (select content_id as id, sum(vote) as vote from ipvotes group by content_id ORDER BY vote desc limit 10) t on contents.id=t.id where vote>0");
  	@vote = true
  	render(:action => 'index')    
  end
